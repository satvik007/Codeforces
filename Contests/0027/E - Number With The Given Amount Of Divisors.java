...
public class Main {
    ...
    static class satvik {
        ...
        long dp[][] = new long [1010][12];
        long primes[] = new long [12];
        long p[][] = new long [12][100];
        ...
        public void solve(int testNumber, InputReader in, BufferedWriter out) throws IOException {
            ...
            for(int i = 1; i <= 10; i++) {
                long bound = (long) 3e18;
                long base = 1;
                for(int j = 0; bound > 0; j++) {
                    p[i][j] = base;
                    base *= primes[i];
                    bound /= primes[i];
                }
            ...
            for(int i = 1; i <= n; i++) {
                for(int j = 1; j <= 10; j++) {
                    ...
                    for(int k = 0; k + 1 <= i; k++) {
                        if(p[j][k] == 0) break;
                        if(i % (k + 1) != 0 || bound / p[j][k] < dp[i / (k + 1)][j - 1]) {
                            continue;
                        }
                        dp[i][j] = min(dp[i][j], dp[i / (k + 1)][j - 1] * p[j][k]);
                    }
                }
            }

            out.write(Long.toString(dp[n][10]));
